...
class CNINFO(Crawler):
    ...
    def get_report(self, savepath, dbqueue=None):
        ...
        for bblx in ReportType:
            try:
                fn = '{}_{}_{}_{}_{}.zip'.format(ret[0]['market'], bblx.name, self.code, ret[0]['startTime'], time.localtime().tm_year)
                ...
                for row in csv.reader(content.splitlines()):
                    if ishead:
                        ...
                        if len(row[0]) > 0:  # code is not null
                            ops.append((sql, row))
                            conn = sqlite.connect("E:/stock/cnstock.db")
                            for op in ops:
                                print(op)
                                conn.execute(op[0], op[1])
                            conn.commit()
                            conn.close()
                            insertedrows += 1
...
class Sina(Crawler):
    ...
    def get_sharebonushtml(self):
        ...
        url = 'http://vip.stock.finance.sina.com.cn/corp/go.php/vISSUE_ShareBonus/stockid/{}.phtml'.format(self.code)
        return get_read(url)
...
if __name__ == '__main__':
    ...
    # CNINFO('834780').get_3b_anno('e:/stock/3BAnno')
    # CNINFO('600481').get_report('e:\stock\CNReport')
    # Sina('600481').get_manager()
    # Sina("000001").get_holder()
    # Sina('000890').get_brief2()
    # Sina('600481').get_sharebonus()
    ...
    for stock in get_stocklist(arr2):
        print(stock)
        sCode = stock['code']
        # time.sleep(20)
        Sina(sCode).get_holder()
        # time.sleep(10)
        CNINFO(sCode).get_report('e:\stock\CNReport')
        # time.sleep(10)
        Sina(sCode).get_sharebonus()
        # time.sleep(10)
        CNINFO(sCode).get_3b_anno('e:/stock/3BAnno')
        # time.sleep(10)
        Sina(sCode).get_brief2()
        # time.sleep(10)
        CNINFO(stock['code'], stock['zwjc'], stock['orgId']).get_brief('Anno')
        # time.sleep(10)
        JRJ(sCode).get_lift_ban()
        # time.sleep(10)
        ops = Sina(sCode).get_manager()
        conn = sqlite.connect("E:/stock/cnstock.db")
        for op in ops:
            conn.execute(op[0], op[1])
        conn.commit()
